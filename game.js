...
export const game = {
  phase: 'menu',       // menu | play | roundover | matchover | wavebreak
...
// 콜백 (UI 연결용)
export const hooks = { onBanner: null, onHud: null, onModeEnd: null };
...
function tryShoot() {
  ...
  const reload = me.rapid > 0 ? RELOAD_MS*0.4 : RELOAD_MS;
  ...
  if (NET.connected && !NET.isHost) {
    ...
    spawnBullet(game.myId, bx, bz, dir);
  }
}
...
function startRound() {
  ...
  hooks.onBanner && hooks.onBanner('라운드 '+game.round, '#59e0ff', 1200);
}

function roundEnd(winnerId) {
  if (game.phase !== 'play') return;
  game.phase = 'roundover';
  if (winnerId === game.myId) game.scoreMe++; else game.scoreFoe++;
  const iWon = winnerId === game.myId;
  hooks.onBanner && hooks.onBanner(iWon?'라운드 승리!':'라운드 패배', iWon?'#74f0a7':'#ff6b8f', 1800);
  if (NET.connected && NET.isHost)
    NET.sendFn && NET.sendFn({ t:'round', winner:winnerId, scoreP1:game.scoreMe, scoreP2:game.scoreFoe });
  if (game.scoreMe >= 2 || game.scoreFoe >= 2) {
    setTimeout(() => matchEnd(game.scoreMe >= 2), 1900);
  } else {
    game.round++;
    setTimeout(() => { if (game.phase==='roundover') startRound(); }, 2000);
  }
}

export function handleRoundEvent(msg) {
  const iWon = msg.winner === game.myId;
  game.phase = 'roundover';
  game.scoreMe = game.myId==='p1'?msg.scoreP1:msg.scoreP2;
  game.scoreFoe = game.myId==='p1'?msg.scoreP2:msg.scoreP1;
  hooks.onBanner && hooks.onBanner(iWon?'라운드 승리!':'라운드 패배', iWon?'#74f0a7':'#ff6b8f', 1800);
  if (game.scoreMe >= 2 || game.scoreFoe >= 2) setTimeout(()=>matchEnd(game.scoreMe>=2),1900);
  else { game.round++; setTimeout(()=>{game.phase='play';},2000); }
}
...
function coopWaveComplete() {
  ...
  game.coopScore += game.wave * 100;
  hooks.onBanner && hooks.onBanner('웨이브 '+game.wave+' 클리어! +'+(game.wave*100), '#74f0a7', 2000);
  game.wave++;
  if (NET.connected && NET.isHost) NET.sendFn && NET.sendFn({ t:'wave', wave:game.wave, score:game.coopScore, lives:game.coopLives });
  setTimeout(()=>{ if (game.phase==='wavebreak') startCoopWave(); }, 2500);
}
...
function onTankDeath(deadTank, killerId) {
  if (game.mode === 'versus') {
    ...
    if (deadTank.id.startsWith('e')) {
      ...
      if (idx>=0){ setTimeout(()=>{ scene.remove(deadTank.mesh); },100); }
      if (enemies.every(e=>!e.alive)) coopWaveComplete();
    } else {
      // 플레이어 사망 -> 목숨 감소 후 리스폰 or 게임오버
      game.coopLives--;
      if (game.coopLives<=0) { coopGameOver(); }
      else {
        setTimeout(()=>{ deadTank.hp=MAX_HP; deadTank.alive=true; deadTank.x=(Math.random()-0.5)*20; deadTank.z=(Math.random()-0.5)*20; }, 1500);
      }
    }
  }
}
...
export function handleWaveEvent(msg){ game.wave=msg.wave; game.coopScore=msg.score; game.coopLives=msg.lives; }
  ...
  if(game.phase==='play'){
    ...
    if(hostSide){
      ...
      if(game.mode==='versus'){
        ...
        enemies = enemies.filter(e=>{ if(!e.alive && e.mesh.parent===null) return false; return true; });
      }
      updateBullets(dt);
      applyTransforms();
...
export function shootFromNet(msg){ spawnBullet(msg.ownerId || 'p2', msg.x, msg.z, msg.dir); }
